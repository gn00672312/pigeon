...
class LogStream(object):
    ...
    def _determine_log_info(cls):
        ...
        _dir = os.getenv('LOG_DIR') or\
               LogConfigParser.LOG_DIR or\
               cls.log_dir or os.getcwd()

        if not os.path.isabs(_dir):
            _dir = os.path.abspath(_dir)

        _name = os.getenv('LOG_NAME') or\
    ...
    def _determine_collective_info(cls):
        ...
        _dir = os.getenv('COLLECTIVE_DIR') or\
               LogConfigParser.COLLECTIVE_DIR or\
               cls.collective_dir or\
               os.getcwd()

        if not os.path.isabs(_dir):
            _dir = os.path.abspath(_dir)

        _name = os.getenv('COLLECTIVE_NAME') or\
                LogConfigParser.COLLECTIVE_NAME or\
                cls.collective_name or\
    ...
    def _get_file_path(cls, for_collective, create_dir=True):
        ...
        # Make final version.
        full_path += datetime.date.today().strftime('.%Y-%m-%d')
