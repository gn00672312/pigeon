...
class LogConfigParser(object):
    ...
    def parse_log_config(cls):
        ...
        # First priority: using environment variable.
        if os.getenv('LOG_CONFIG'):
        ...
        # Second priority: 'log.conf' under current working directory.
        if os.path.isfile(os.path.join(os.getcwd(), 'log.conf')):
            config_path = os.path.join(os.getcwd(), 'log.conf')
            try:
                config_file = open(config_path)
            except Exception:
        ...
        # Third, last, priority: programmatically.
